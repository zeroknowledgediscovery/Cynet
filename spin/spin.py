...
def showGlobalPlot(coords,ts=None,fsize=[14,14],cmap='jet',m=None,figname='fig',F=5):
    """
    ...
    if num is not None:
        m.scatter(lons, lats,s=4**(4.25+F*(num)),
                  c=num,
                  cmap=cmap,alpha=.7,
                  norm=colors.LogNorm(vmin=np.min(num), vmax=np.max(num)),linewidth=2,edgecolor='w',
                  zorder=5)
    else:
    ...
    if num is not None:
        ...
        cbar = mpl.colorbar.ColorbarBase(cax, cmap=colormap,orientation='horizontal',
