...
class spatioTemporal:
    ...
    def __init__(self,
        ...
        # at this point the column name corresponding to date
        # will be stored in the variable DATE
        else:
            # but all bets are off here, b/d date column can be called anything
            # Assuming that date column will be 'Date' as per the DATE variable,
            # but must either have
        ...
        if grid is not None:
            if isinstance(grid, dict):
                ...
                assert('Eps' in grid)
                # constructing private variable self._grid
    ...
    def getTS(self,_types=None,tile=None):
        ...
        TS_NAME = ('#'.join(str(np.around(x,decimals=PRECISION))
                            for x in tile))+"#"+stringify(_types)
...
def readTS(TSfile,csvNAME='TS1',BEG=None,END=None):
    ...
    Reads in output TS logfile into pandas DataFrame
        and then outputs necessary
        CSV files in XgenESeSS-friendly format.
        Three CSV files are produced:
        1. csvNAME.csv (data in rows)
        2. csvNAME.coords (coordinates in column)
        3. csvNAMES.columns (dates in row)

    Input -
        TSfile (string): filename input TS to read
               or list of TSfile (list of strings)
        csvNAME (string)
...
def splitTS(TSfile,dirname='./',prefix="@",
    ...
    Input -
        TSfile (string): filename input TS to read
               or list of TSfile (list of strings)
        dirname: path to directory to store files
        prefix: string to preprend to file names
        VARNAME: string to append to file names
        BEG (string): start datetime
        END (string): end datetime
